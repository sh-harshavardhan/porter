...
class PorterConfig(BaseModel):
    ...
    description: Optional[str] = Field(
        None,
        description="A brief description of the Porter job"
    )
    ...
    pre_validations: Optional[Validation] = Field(
        None,
        description="List of validations to be run before the transforms"
    )
    pre_transformations: Optional[Transform] = Field(
        None,
        description="List of transformations to be run before the main transforms"
    )
    ...
    custom_write_options: Optional[List[CustomWriteOptions]] = Field(
        ...
        examples=[
            [{'target': 'postgres', 'dataset_name': 'sales_data', 'target_name': 'sales_2023'}],
            [{'target': 's3', 'dataset_name': 'customers', 'target_name': 'customers_current', 'truncate_target': True}]
        ],
    )
    ...
    metadata: Optional[Dict] = Field(
        ...,
        description="Metadata for the Porter job"
    )
